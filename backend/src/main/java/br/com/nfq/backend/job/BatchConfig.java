...
public class BatchConfig {
    ...
    Step step(ItemReader<TransacaoCNAB> reader, ItemProcessor<TransacaoCNAB, Transacao> processor, ItemWriter<Transacao> writer) {
        return new StepBuilder("step", jobRepository).<TransacaoCNAB, Transacao>chunk(1000).transactionManager(transactionManager).reader(reader).processor(processor).writer(writer).build();
    }

    /**
     * Leitura de dados de tamanho fixo (Formato CNAB)
     *
     * @return mapper para TransacaoCNAB
     */
    @Bean
    @StepScope
    //para acessar o JobParameters
    FlatFileItemReader<TransacaoCNAB> reader(@Value("#{jobParameters['cnabFile']}") Resource file) {
        return new FlatFileItemReaderBuilder<TransacaoCNAB>().name("reader").resource(file).fixedLength().columns(new Range(1, 1), new Range(2, 9), new Range(10, 19), new Range(20, 30), new Range(31, 42), new Range(43, 48), new Range(49, 62), new Range(63, 80)).names("tipo", "data", "valor", "cpf", "cartao", "hora", "donoLoja", "nomeLoja").targetType(TransacaoCNAB.class).build();
    }

    @Bean
    ItemProcessor<TransacaoCNAB, Transacao> processor() {
        //Wither patter
        return item -> new Transacao(null, item.tipo(), null, null, item.cpf(), item.cartao(), null, item.donoLoja(), item.nomeLoja()).withValor(item.valor().divide(BigDecimal.valueOf(100), RoundingMode.UNNECESSARY)).withData(item.data()).withHora(item.hora()).withSinal(item.tipo());
    }
